...
class MSupplier extends Model
{
    ...
    public function getListBundleDt($supplier_cd=""){
        ...
        if(!empty($supplier_cd)){
            $mSupplier = $mSupplier->select('bundle_dt')->where('mst_suppliers_cd','LIKE','%'.$supplier_cd.'%')->whereNull('deleted_at')->whereNotNull('bundle_dt')->groupBy('bundle_dt');
        }else{
            $mSupplier = $mSupplier->select('bundle_dt')->distinct('bundle_dt')->whereNull('deleted_at')->whereNotNull('bundle_dt')->orderBy('bundle_dt');
        }
