<div v-bind:class="errors.{!! $filed !!} != undefined ? 'error-form':'' ">
    <div class="wrap-control-group {!! isset($class) ? $class:"" !!}">
        ...
        <input  type="text" v-bind:class="errors.{!! $filed !!} != undefined ? 'form-control is-invalid':'form-control' "
               class="form-control"
               v-model="image_{!! $filed !!}" readonly style="background-color: white!important; {{isset($role) && $role==2 ? ';background-color: #D9D9D9;opacity: 1;' : ''}}">
    </div>
...
<div class="wrap-control-group">
    ...
    <label  class="btn label-file-image" for="{!! $filed !!}">{{trans($prefix.'btn_browse_license_picture')}}</label>
